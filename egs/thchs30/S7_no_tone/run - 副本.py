...
def preare_data(data_src):
	...
	'''
	#生成train lab
	lab_list=get_suffix(data_src+r'\train','.txt')
	for line in lab_list:
		lab=open(line,'r').read()
		lab=remove_sign(lab,'\d,.!?\\~\/:;@$%=+"').upper()
		open(line.replace('.txt','.lab'),'w').write(lab[2:])
		#可能处理一些特殊单词如 'EM
	...
	#补充dict缺省的单词
	#替换trainwords
	cmd=r'HLEd -l * -d data\dict\dict -i data\phones\trainphones.mlf '\
...
if __name__ == "__main__":
	...
	#os.system(cmd)
	#统计均值和方差
	if not os.path.exists(r'hmms\hmm0') : os.mkdir(r'hmms\hmm0 ')
	...
	'''
	#iter 1
	if not os.path.exists(r'hmms\hmm1') : os.mkdir(r'hmms\hmm1')
	mono_model(r'data\phones\monophones1',r'hmms\hmm0\proto',r'hmms\hmm0\vFloors')
	...
	os.system(cmd)
	#iter 2
	if not os.path.exists(r'hmms\hmm2') : os.mkdir(r'hmms\hmm2')
	cmd=r'HERest -A -D -T 1 -C data\conf\config -I data\phones\trainphones.mlf -t 250.0 150.0 1000.0 '+\
	...
	cmd=r'HVite -l * -o SWT -b  SILENCE -C data\conf\config -a -H hmms\hmm7\macros -H hmms\hmm7\hmmdefs_sp '+\
	r'-i data\train\aligned.mlf -m -t 250.0 -y lab -I data\train\trainwords.mlf -S data\train\train.scp '+\
	r'data\dict\dict data\phones\monophones1'
	os.system(cmd)
	#iter 1
	if not os.path.exists(r'hmms\hmm8') : os.mkdir(r'hmms\hmm8')
	cmd=r'HERest -A -D -T 1 -C data\conf\config -I data\phones\aligned.mlf -t 250.0 150.0 1000.0 '+\
	r'-S data\train\train.scp -H hmms\hmm7\macros -H hmms\hmm7\hmmdefs_sp -M hmms\hmm8 data\phones\monophones1'
	print cmd
	os.system(cmd)
	...
	cmd=r'HERest -A -D -T 1 -C data\conf\config -I data\phones\aligned.mlf -t 250.0 150.0 1000.0 '+\
	r'-S data\train\train.scp -H hmms\hmm8\macros -H hmms\hmm8\hmmdefs_sp -M hmms\hmm9 data\phones\monophones1'
	print cmd
	os.system(cmd)
	
	#解码 ps. 需要lab文件，注意文件中的特殊单词
	cmd=r'HVite -H hmms\hmm9\macros -H hmms\hmm9\hmmdefs_sp -S data\test\test.scp -f -l * -i result\recout_mono.mlf '+\
	r'-w data\lang\wdnet -p 0.0 -s 5.0 data\dict\dict data\phones\monophones1'
	os.system(cmd)
	#评分 ps.注意试试 testwords只保留文件名，不保留父目录
	
	cmd=r'HResults -I data\test\testwords.mlf data\phones\monophones1 result\recout_mono.mlf'
